    ...
    internal static class TransitionStringBuilder
    {
        internal static string GetUncheckedVisited(string scene)
        {
            ...
            IEnumerable<string> uncheckedTransitions = RM.RS.TrackerData.uncheckedReachableTransitions
                .Where(t => TD.TryGetScene(t, out string s) && s == scene);
            ...
            if (uncheckedTransitions.Any())
            {
                ...
                {
                    if (TD.GetTransitionDef(transition) is not TransitionDef td) continue;
            ...
            var visitedTransitions = RM.RS.TrackerData.visitedTransitions
                .Where(t => TD.TryGetScene(t.Key, out string s) && s == scene)
            ...
            var visitedTransitionsTo = RM.RS.TrackerData.visitedTransitions
                .Where(t => TD.TryGetScene(t.Value, out string s) && s == scene)
                .ToDictionary(t => TD.GetTransitionDef(t.Key), t => TD.GetTransitionDef(t.Value));
            ...
            var vanillaTransitions = RM.RS.Context.Vanilla
                .Where(t => RD.IsTransition(t.Location.Name)
                    && TD.TryGetScene(t.Location.Name, out string s) && s == scene)
            ...
            var vanillaTransitionsTo = RM.RS.Context.Vanilla
                .Where(t => RD.IsTransition(t.Location.Name)
                    && TD.TryGetScene(t.Item.Name, out string s) && s == scene
        ...
        private static string BuildTransitionStringList(Dictionary<TransitionDef, TransitionDef> transitions, string subtitle, bool to, bool addNewLines)
        {
            string text = "";
