...
namespace RandoMapMod.UI
{
    internal class ProgressHintMainUpdater : MainUpdater
    {
        ...
        private bool terminateOnNewProgress = false;

        public ProgressHintMainUpdater(TrackerData td, ProgressionManager newPM) : base(td.lm, newPM)
        {
            ...
            AddEntries(td.ctx.Vanilla.Select(v => new DelegateUpdateEntry(v.Location, pm =>
            {
            ...
            AddEntries(td.ctx.itemPlacements.Select((p, id) => new DelegateUpdateEntry(p.Location.logic, (pm) =>
            {
            ...
            AddEntries(td.ctx.transitionPlacements.Select((p, id) => new DelegateUpdateEntry(p.Source, (pm) =>
            {
                if (NewProgressFound) return;
            ...
            foreach (int i in td.obtainedItems)
            {
                ...
                (RandoModItem item, RandoModLocation loc) = td.ctx.itemPlacements[i];
                newPM.Add(item, loc);
            }
            ...
            foreach (KeyValuePair<string, string> kvp in td.visitedTransitions)
            {
                ...
                LogicTransition tt = td.lm.GetTransitionStrict(kvp.Value);
                LogicTransition st = td.lm.GetTransitionStrict(kvp.Key);
        ...
        private class TrackedStateUpdateEntry(ProgressHintMainUpdater mu, Term term, StateLogicDef logic) : StateUpdateEntry(term, logic)
        {
            private readonly List<State> stateAccumulator = [];
        ...
        private class TrackedPreplacedItemUpdateEntry(ProgressHintMainUpdater mu, ILogicItem item, ILogicDef location) : PrePlacedItemUpdateEntry(item, location)
        {
