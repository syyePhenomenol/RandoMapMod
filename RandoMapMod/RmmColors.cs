...
{
    internal static readonly RmmColorSetting[] PinBorderColors =
    [
    ...
    private static Dictionary<RmmColorSetting, Vector4> _customColors = [];

    private static readonly Dictionary<RmmColorSetting, Vector4> _defaultColors =
        new()
        {
    ...
    public override void OnEnterGame()
    {
        ...
        try
        {
            customColorsRaw = JsonUtil.DeserializeFromExternalFile<Dictionary<string, float[]>>(
                Path.Combine(Path.GetDirectoryName(RandoMapMod.Assembly.Location), "colors.json")
            );
        }
        catch (Exception)
        {
        ...
        if (customColorsRaw is not null)
        {
            ...
            {
                if (
                    !Enum.TryParse(colorSettingRaw, out RmmColorSetting colorSetting)
    ...
    {
        if (_customColors is not null && _customColors.ContainsKey(rmmColor))
        {
            return _customColors[rmmColor];
        }

        if (_defaultColors.ContainsKey(rmmColor))
        {
            return _defaultColors[rmmColor];
        }
